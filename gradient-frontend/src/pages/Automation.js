...
const getLeadStatus = (lead) => lead?.status || 'новий';
...
const useLeadData = (snapshot, updateSnapshot) => {
  ...
  const refresh = useCallback(async (options = {}) => {
    ...
    setError(null);
    try {
      const data = await getGmailLeads(null, { lightweight: true });
...
const normalizeEmailStatus = (status) => {
  const raw = String(status || 'новий').trim().toLowerCase();
  const map = {
    'новий': 'new',
    new: 'new',
    waiting: 'new',
...
const getStatusLabel = (status) => {
  ...
  if (directLabels[raw]) return directLabels[raw];
  const key = normalizeEmailStatus(status);
  return BADGE_LABELS[key] || raw || 'Новий';
};

const getStatusBadgeVariant = (status) => {
  const key = normalizeEmailStatus(status);
  if (key === 'call_lead') return 'call_lead';
  if (key === 'snoozed') return 'snoozed';
  if (['confirmed', 'rejected', 'postponed', 'in_work', 'restored', 'waiting'].includes(key)) {
    return key;
  }
  return 'new';
};
...
const Automation = () => {
  ...
  const filteredLeads = useMemo(() => {
    ...
    return visibleLeads.filter((lead) => {
      ...
      const badgeVariantForFilter =
        status === 'call_lead'
          ? 'call_lead'
          : ['confirmed', 'rejected', 'snoozed', 'postponed', 'restored', 'in_work'].includes(status)
            ? status === 'postponed' ? 'snoozed' : status
            : status === 'waiting'
              ? 'waiting'
              : qualified
                ? 'qualified'
  ...
  const handleEmailAction = useCallback(
    async (action) => {
      ...
      const msgId = selectedLead.gmail_id || selectedLead.gmailId;
      if (!msgId) {
        setStatusError('Не знайдено ідентифікатор листа.');
        return null;
      }
      ...
      try {
          ...
          updateLeadSnapshot(
            {
              ...
              leads: data.leads.map((lead) =>
                getLeadKey(lead) === getLeadKey(selectedLead) ? { ...lead, ...updated } : lead
              ),
            },
        ...
        pushNotification({
          variant: 'success',
          title: 'Статус оновлено',
          message: `Статус змінено на "${getStatusLabel(updated.status)}"`,
        });
    ...
    [selectedLead, data, updateLeadSnapshot, refresh, pushNotification, closeLocalModal, navigate]
  );

  const handleConfirmClick = useCallback(async () => {
    if (!selectedLead || replyLoading || actionLoading) return;

    const updated = await handleEmailAction('confirm');
    if (!updated) return;
  ...
  }, [selectedLead, replyLoading, actionLoading, handleEmailAction, handleGenerateReplies, replyStyle]);

  const renderEmailActionButtons = () => {
    if (!selectedLead) return null;

    const normalized = normalizeEmailStatus(selectedLead.status);
    const assignedManagerId = getAssignedManagerId(selectedLead);
    const currentUserId = user?.id;
    const isOwnedByCurrentUser =
      assignedManagerId != null &&
      currentUserId != null &&
      Number(assignedManagerId) === Number(currentUserId);
    ...
    if (normalized === 'rejected') {
      return (
        <ActionButton
          type="button"
          $variant="confirm"
          onClick={() => handleEmailAction('restore')}
          disabled={actionLoading}
        >
          {actionLoading ? 'Оновлюємо...' : 'Поновити'}
        </ActionButton>
      );
    }

    const showStandardActions =
      ['new', 'confirmed', 'postponed', 'restored'].includes(normalized) ||
      (normalized === 'in_work' && isOwnedByCurrentUser);
    const showBlockedActions = normalized === 'in_work' && isTakenByOther;

    if (!showStandardActions && !showBlockedActions) {
      return null;
    }

    const takeWorkDisabled =
      actionLoading ||
      showBlockedActions ||
      (normalized === 'in_work' && isOwnedByCurrentUser);
    const takeWorkLabel =
      normalized === 'in_work' && isOwnedByCurrentUser ? 'У вас в роботі' : 'Взяти в роботу';
    const secondaryDisabled = actionLoading || showBlockedActions;
    ...
    return (
      <>
        <ActionButton
          type="button"
          $variant="generate"
          onClick={() => handleEmailAction('take_to_work')}
          disabled={takeWorkDisabled}
        >
          {takeWorkLabel}
        </ActionButton>
        <ActionButton
          type="button"
          $variant="snooze"
          onClick={() => handleEmailAction('postpone')}
        ...
        <ActionButton
          ...
          disabled={secondaryDisabled || replyLoading}
        >
          {actionLoading ? 'Оновлюємо...' : replyLoading ? 'Генеруємо...' : 'Підтвердити'}
        </ActionButton>
      </>
    );
  };
            ...
            <LeadsTable>
              ...
              <tbody>
                ...
                {filteredLeads.map((lead, index) => {
                  ...
                  const badgeVariant = getStatusBadgeVariant(leadStatus);
                  const badgeLabel = getStatusLabel(leadStatus);

                  const rowStyle = DECISION_ROW_TONES[normalizeEmailStatus(leadStatus)]

                    ? { '--row-bg': DECISION_ROW_TONES[normalizeEmailStatus(leadStatus)] }

                    : normalizeEmailStatus(leadStatus) === 'new'
              ...
              <ModalHeader>
                ...
                <ModalTitleRow>
                  ...
                  <StatusBadge
                    ...
                    title={STATUS_TOOLTIPS[getStatusBadgeVariant(selectedLead.status)] || STATUS_TOOLTIPS.new}
                  >
